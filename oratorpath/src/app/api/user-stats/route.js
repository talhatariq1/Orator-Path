...
export async function GET() {
  try {
    ...
    // Check cache first
    const cacheKey = generateCacheKey(userId, 'user-stats');
    ...
    const queryResult = await handleMongoQuery(
      async () => {
        return await AudioAnalysis.find({ clerkId: userId })
          .sort({ analysisDate: -1 })
    ...
    const responseData = { ...stats, performanceData };
...
function calculatePerformanceData(analyses) {
  ...
  const sessions = recentAnalyses.map(analysis => {
    ...
    // Get day name for dates within the last week
    else {
    ...
    const confidenceScore = confidenceFactors.length > 0
      ? Math.round(confidenceFactors.reduce((sum, score) => sum + score, 0) / confidenceFactors.length)
      : Math.round(overallScore * 0.85); // Fallback to a percentage of overall score
    ...
    const vocabularyScore = vocabularyFactors.length > 0
      ? Math.round(vocabularyFactors.reduce((sum, score) => sum + score, 0) / vocabularyFactors.length)
      : Math.round(overallScore * 0.95); // Fallback to a percentage of overall score
...
function calculateUserStats(analyses) {
  ...
  const scoresArray = analyses
    .filter(analysis => analysis.recommendations?.performance_assessment?.overall_score)
    .map(analysis => analysis.recommendations.performance_assessment.overall_score);
  ...
  const coherenceScores = analyses
    .filter(analysis => analysis.transcriptAnalysis?.coherence_score !== undefined)
    .map(analysis => analysis.transcriptAnalysis.coherence_score);
  ...
  if (coherenceScores.length > 0) {
    ...
    const hasScores = analyses.some(analysis =>
      analysis.recommendations?.performance_assessment?.overall_score !== undefined
    );
  ...
  if (analyses.length <= 3) {
    ...
    if (analyses.length === 1) {
      ...
      const olderCoherenceScores = older
        .filter(analysis => analysis.transcriptAnalysis?.coherence_score !== undefined)
        .map(analysis => analysis.transcriptAnalysis.coherence_score);

      const olderCoherenceAvg = olderCoherenceScores.length > 0
        ? olderCoherenceScores.reduce((sum, score) => sum + score, 0) / olderCoherenceScores.length
        : 0;
    ...
    const recentCoherenceScores = recent
      .filter(analysis => analysis.transcriptAnalysis?.coherence_score !== undefined)
      .map(analysis => analysis.transcriptAnalysis.coherence_score);
    ...
    // Calculate averages
    const recentCoherenceAvg = recentCoherenceScores.length > 0
      ? recentCoherenceScores.reduce((sum, score) => sum + score, 0) / recentCoherenceScores.length
      : 0;
  ...
  const practiceTimeChange = previousPracticeTime > 0
    ? Math.round(((totalPracticeTime - previousPracticeTime) / previousPracticeTime) * 100)
    : 0;

  // Total practice time (all time)
  const allTimePracticeTime = Math.round(
  ...
  const lastUpdated = mostRecentAnalysis ?
    formatDate(new Date(mostRecentAnalysis.analysisDate)) : "Never";
