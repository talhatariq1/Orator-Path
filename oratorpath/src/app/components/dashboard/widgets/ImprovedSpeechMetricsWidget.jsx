...
const ImprovedSpeechMetricsWidget = ({ metrics, title = "Speech Metrics" }) => {
  ...
  const calculateLabelPosition = (angle, labelText) => {
    // Adjust label radius based on text length for better spacing
    const baseLabelRadius = radius + 40;
    const labelRadius = baseLabelRadius + (labelText.length > 8 ? 10 : 0);
    
    const x = centerX + labelRadius * Math.cos(angle);
    const y = centerY + labelRadius * Math.sin(angle);
    
    // Determine text anchor and adjustments based on angle position
    let textAnchor = "middle";
    let dx = 0;
    let dy = 0;
    let bgWidth = labelText.length * 7 + 10; // Dynamic width based on text length
    let bgX = 0;
        ...
        <div className={`flex justify-center transition-all duration-500 ${isExpanded ? 'h-[350px]' : 'h-[280px]'}`}>
          ...
          >
            <svg width="100%" height="100%" viewBox="0 0 240 240" preserveAspectRatio="xMidYMid meet">
              ...
              {/* Enhanced labels with improved positioning and animations */}
              {points.map((point, i) => {
                const isActive = hoveredMetric === point.label || activeMetric === point.label;
                const labelPosition = calculateLabelPosition(point.angle, point.label);
                ...
                return (
                  <g key={i}>
                    ...
                    {/* Label text */}
                    <motion.text
                      x={labelPosition.x + labelPosition.dx}
                      y={labelPosition.y + labelPosition.dy}
...
export default ImprovedSpeechMetricsWidget;
