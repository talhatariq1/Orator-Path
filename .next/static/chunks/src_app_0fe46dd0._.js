    ...
    const fetchUserStats = async ()=>{
        try {
            ...
            try {
                ...
                clearTimeout(timeoutId); // Clear the timeout if fetch completes
                if (!response.ok) {
                    console.error('User stats API error status:', response.status);
                ...
                if (!data || typeof data.totalPracticeSessions === 'undefined' || typeof data.averageScore === 'undefined' || typeof data.speakingClarityScore === 'undefined') {
                    console.error('Invalid user stats data format:', data);
            ...
            setError(`Failed to load your statistics: ${err.message}`);
        } finally{
        ...
        try {
            ...
            try {
                ...
                if (!response.ok) {
                    ...
                    throw new Error(`Failed to fetch speech metrics: ${response.status} ${response.statusText}`);
                }
                ...
                try {
                    ...
                    if (data.metrics.length === 0) {
                        console.warn('Empty metrics array received, but continuing with processing');
                    }
            ...
            // Set a user-friendly error message
            setError(`Failed to load speech metrics: ${err.message}`);
        } finally{
        ...
        try {
            ...
            try {
                ...
                if (!response.ok) {
                    ...
                    throw new Error(`Failed to fetch practice consistency data: ${response.status} ${response.statusText}`);
                }
                const data = await response.json();
...
//# sourceMappingURL=src_app_0fe46dd0._.js.map