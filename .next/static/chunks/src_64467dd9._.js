 ...
 */ const CustomSignOutButton = ({ redirectUrl = '/', children })=>{
    ...
    const handleSignOut = async ()=>{
        try {
            ...
            setTimeout(()=>{
                // Use replace instead of push to prevent going back to the dashboard after sign-out
                router.replace(redirectUrl);
            }, 300);
        } catch (error) {
            console.error('Error signing out:', error);
            // If there's an error, try to redirect anyway after a short delay
            setTimeout(()=>{
                router.replace(redirectUrl);
            }, 300);
        } finally{
            // Keep loading state for a bit to show the loading indicator
            setTimeout(()=>{
                setIsLoading(false);
            }, 300);
        }
...
_s(CustomSignOutButton, "0zbrvuausSrdBpAwTCjenAC2QZc=", false, function() {
    return [
...
//# sourceMappingURL=src_64467dd9._.js.map